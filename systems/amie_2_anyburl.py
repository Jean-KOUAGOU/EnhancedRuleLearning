...
def read_file(path):
    ...
    content = [x.strip() for x in content if x.startswith('?')]
    return content
...
def amieToAnyBURL(in_path, out_path, pca):
    ...
    for rule in content:
        ...
        Rule, Head_Coverage, Std_Confidence, PCA_Confidence, Positive_Examples, Body_size, PCA_Body_size, Functional_variable = rule.split("\t")
